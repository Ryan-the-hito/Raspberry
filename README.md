...
### 从原生 Launchpad 中去除的功能

1. 拖拽

在原生的 Launchpad 中，用户可以通过拖拽的方式，将两个软件的图标组合在一起，形成一个组别，Raspberry 倾向于使用右键的方式，因此不再添加这个功能。另外，原生的 Launchpad 因为只能通过拖拽添加组别，因此当用户将一个软件拖拽加入位于行尾的组别中时，会变成经典的猫鼠追逐游戏。此外，让用户手动将上百个软件从多页之后手动拖拽到指定组别中，非常耗费精力，不是简洁的设计。

同时，Raspberry 也去除了通过拖拽来改变软件顺序的功能，取而代之的是使用快捷键，这会极大地增添稳定性，以前通过拖拽手势，把一个软件从很后面的页面里拽到前面的事情不会再发生了。

2. 翻页切换动画

原生的 Launchpad 在翻页的时候有平移动画，但这个动画的实际意义可能不大，不如直接快速显示更加有效率。

3. 固定行列位置

原生的 Launchpad 在行数没有达到预期最大值的时候，现有的行都向上靠拢。但 Raspberry 认为这样其实不利于点击。例如默认设定最大行数为 5 行，但现在只有两行软件，应该将这两行软件居中放置才有利于点击。
