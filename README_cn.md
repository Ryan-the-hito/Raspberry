...
因此，Raspberry 在付费版中增加了迁移功能，可以将用户在 macOS 26 之前系统上的启动台分组保存在本地，待用户升级到 macOS 26 之后，打开 Raspberry 即自动完成迁移。（需要注意的是：**此功能需要在升级前就购买并安装 Raspberry 的付费版并正确操作**）
...
首次启动时，Raspberry 将索引电脑中的 app，这个索引不基于 Spotlight，因此即便 Spotlight 索引消失，也保存在本地不丢失。
...
双击功能在经过几周的测试后似乎显得相当具有实验性。我收到的许多反馈显示他们想要一个更传统的，就像 Launchpad 一模一样的操作习惯；因此，从 v0.0.10 版本开始，原生的单击方式与 UI 交互将取代双击方式作为默认模式。如果您更喜欢双击模式（当仅使用键盘控制整个 UI 时更为精确），您可以在左上角的状态菜单中找到一个开关，并使用这个开关来切换不同的点击模式。
...
1. 启动 Raspberry 应用，**首次启动时，它将索引所有应用的图标缓存，请勿退出应用，这可能需要几分钟。请耐心等待。此后正常使用不会出现此现象。**

![ZA8lpNn](https://i.imgur.com/ZA8lpNn.png)

2. 需注意：**【⚠️⚠️⚠️在非传统模式中】双击不同区域对应不同功能效果：**
...
4. 如果您打算将布局从 macOS 15 迁移到 macOS 26，**强烈建议您在安装此应用之前不要更新您的系统**。相反，用户应：

- 在 macOS 15 上安装并启动 Raspberry；
- 点击“将 Launchpad 组备份到 Raspberry”；
- 将您的系统更新到 macOS 26；
- 启动 Raspberry，开始使用。

5. ⚠️⚠️⚠️**如果您想修复可能存在的问题，请在更新软件之后，前往菜单栏左上角，选择“🗑 重置所有数据并重启”项，接着再执行一次备份操作，这样备份就可以顺利进行了。**请按照以下指示操作：

首先，请检查GitHub页面（免费）或您的邮箱（付费）以获取最新的dmg文件。

其次，安装最新应用程序，打开dmg文件，将其拖动到应用程序文件夹中以替换旧版本。

第三，启动Raspberry应用程序，转到菜单栏的左上角，选择“🗑 重置所有数据并重启”选项。

最后，再备份一次试试看。应该可以顺利执行了。

## 注意事项

- 更新应用：这就像安装应用一样。不过用户在下载并打开DMG文件后，必须选择“替换”。
- 请定期备份分组数据，防止意外丢失。
...
1. 备份恢复旧 Launchpad 的时候只能获取第一页的文件夹的问题（Pro version）
2. 为整体增加传统点击模式，默认开启，交互方式和 Launchpad 完全一致，只需要单击点点点就行了，不需要用户再去这里双击那里双击了。
3. 给主界面消失的过程增加渐变退出动画。

### v0.0.9

- 添加了一个信息窗口，用于在首次执行时显示 Raspberry 索引应用程序的信息。
